...
class PMA_Table
{
    ...
    static function isView($db = null, $table = null)
    {
    ...
    static function sGetToolTip($db, $table)
    {
        return PMA_Table::sGetStatusInfo($db, $table, 'Comment') 
            . ' (' . PMA_Table::countRecords($db, $table, true) . ')';
    }

    static function sGetStatusInfo($db, $table, $info = null, $force_read = false)
    {
     ...
     * @access  public
     */
    static function countRecords($db, $table, $ret = false,
     ...
     */
    static function generateAlter($oldcol, $newcol, $type, $length,
     ...
     */
    static function duplicateInfo($work, $pma_table, $get_fields, $where_fields,
      $new_fields)
    {
        $last_id = -1;
     ...
     */
    static function moveCopy($source_db, $source_table, $target_db, $target_table, $what, $move, $mode)
    {
        global $err_url;
        ...
        // end if ($move)
            ...
            if ($what != 'dataonly' && !isset($maintain_relations)) {
                ...
                $where_fields = ['db_name' => $source_db, 'table_name' => $source_table];
                $new_fields = ['db_name' => $target_db, 'table_name' => $target_table];
                 ...
                 */
                $get_fields = ['master_field', 'foreign_table', 'foreign_field'];
                $where_fields = ['master_db' => $source_db, 'master_table' => $source_table];
                $new_fields = ['master_db' => $target_db, 'foreign_db' => $target_db, 'master_table' => $target_table];
                PMA_Table::duplicateInfo('relwork', 'relation', $get_fields, $where_fields, $new_fields);


                $get_fields = ['foreign_field', 'master_table', 'master_field'];
                $where_fields = ['foreign_db' => $source_db, 'foreign_table' => $source_table];
                $new_fields = ['master_db' => $target_db, 'foreign_db' => $target_db, 'foreign_table' => $target_table];
                PMA_Table::duplicateInfo('relwork', 'relation', $get_fields, $where_fields, $new_fields);
    ...
    public function getUniqueColumns($quoted = true)
    {
        ...
        $uniques = PMA_DBI_fetch_result($sql, ['Key_name', null], 'Column_name');
