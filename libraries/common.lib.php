...
function PMA_formatSql($parsed_sql, $unparsed_sql = '')
{
    ...
    switch ($cfg['SQP']['fmtType']) {
        case 'none':
            if ($unparsed_sql != '') {
                $formatted_sql = "<pre>\n" . PMA_SQP_formatNone(['raw' => $unparsed_sql]) . "\n</pre>";
            } else {
...
function PMA_showMessage($message, $sql_query = null, $type = 'notice')
{
    ...
    if ($cfg['ShowSQL'] == true && ! empty($sql_query)) {
        ...
        if (! empty($cfg['SQLQuery']['Edit']) && ! $query_too_big) {
            ...
            $edit_link = ' [' . PMA_linkOrButton($edit_link, $GLOBALS['strEdit'], ['onclick' => $onclick]) . ']';
        } else {
            $edit_link = '';
        }
...
function PMA_formatNumber($value, $length = 3, $comma = 0, $only_down = false)
{
    ...
    if ($length === 0) {
        return str_replace([',', '.'],
                [$GLOBALS['number_thousands_separator'], $GLOBALS['number_decimal_separator']],
 ...
 * @access  public
 */
function PMA_getTab($tab, $url_params = [])
{
    // default values
    $defaults = [
        'text'      => '',
 ...
 */
function PMA_linkOrButton($url, $message, $tag_params = [],
