...
function PMA_displayTableBody(&$dt_result, &$is_display, $map, $analyzed_sql) {
    ...
    $vertical_display['data']       = [];
    $vertical_display['row_delete'] = [];
            ...
            //  b l o b
                ...
                if (stristr($field_flags, 'BINARY')) {
                    ...
                    if (!isset($row[$i]) || is_null($row[$i])) {
                        ...
                        $row[$i]     = ($default_function != $transform_function ? $transform_function($row[$i], $transform_options, $meta) : $default_function($row[$i], [], $meta));
...
function PMA_displayTable_checkConfigParams()
{
    ...
    if (PMA_isValid($_REQUEST['disp_direction'], ['horizontal', 'vertical', 'horizontalflipped'])) {
    ...
    if (PMA_isValid($_REQUEST['display_text'], ['P', 'F'])) {
    ...
    if (PMA_isValid($_REQUEST['relational_display'], ['K', 'D'])) {
        ...
        if (! strlen($table)) {
            ...
            if ($exist_rel) {
                foreach ($exist_rel AS $master_field => $rel) {
                    ...
                    $map[$master_field] = [$rel['foreign_table'],
                                          $rel['foreign_field'],
                                          $display_field,
                                          $rel['foreign_db']];
                } // end while
...
function PMA_prepare_row_data($mouse_events, $class, $condition_field, $analyzed_sql, $meta, $map, $data, $transform_function, $default_function, $nowrap, $where_comparison, $transform_options) {
    ...
    if (isset($map[$meta->name])) {
        ...
        if (isset($GLOBALS['printview']) && $GLOBALS['printview'] == '1') {
            $result .= ($transform_function != $default_function ? $transform_function($data, $transform_options, $meta) : $transform_function($data, [], $meta)) . ' <code>[-&gt;' . $dispval . ']</code>';
        } else {
            ...
            if ($transform_function != $default_function) {
                ...
                if ('D' == $_SESSION['userconf']['relational_display']) {
                    ...
                    $result .= $transform_function($dispval, [], $meta);
                } else {
                    // otherwise display data in the cell
                    $result .= $transform_function($data, [], $meta);
                }
            }
            $result .= '</a>';
        }
    } else {
        $result .= ($transform_function != $default_function ? $transform_function($data, $transform_options, $meta) : $transform_function($data, [], $meta));
    }
    $result .= '</td>' . "\n";
