...
function PMA_extractPrivInfo($row = '', $enableHTML = FALSE)
{
    $grants = [
        ...
        ['Process_priv', 'PROCESS', $GLOBALS['strPrivDescProcess']],
        ...
        ['Create_view_priv', 'CREATE VIEW', $GLOBALS['strPrivDescCreateView']],
        ['Event_priv', 'EVENT', $GLOBALS['strPrivDescEvent']],
        ['Trigger_priv', 'TRIGGER', $GLOBALS['strPrivDescTrigger']],
        // for table privs:
        ['Create View_priv', 'CREATE VIEW', $GLOBALS['strPrivDescCreateView']],
        ['Show_view_priv', 'SHOW VIEW', $GLOBALS['strPrivDescShowView']],
        // for table privs:
        ['Show view_priv', 'SHOW VIEW', $GLOBALS['strPrivDescShowView']],
        ['Create_routine_priv', 'CREATE ROUTINE', $GLOBALS['strPrivDescCreateRoutine']],
        ['Alter_routine_priv', 'ALTER ROUTINE', $GLOBALS['strPrivDescAlterRoutine']],
        ['Create_user_priv', 'CREATE USER', $GLOBALS['strPrivDescCreateUser']],
        ['Execute_priv', 'EXECUTE', $GLOBALS['strPrivDescExecute5']],
    ];
...
function PMA_displayPrivTable($db = '*', $table = '*', $submit = TRUE)
{
    ...
    if (! empty($columns)) {
        ...
        $privTable[1] = [
            ...
            ['Create_tmp_table', 'CREATE TEMPORARY TABLES', $GLOBALS['strPrivDescCreateTmpTable']],
            ['Show_view', 'SHOW VIEW', $GLOBALS['strPrivDescShowView']],
            ['Create_routine', 'CREATE ROUTINE', $GLOBALS['strPrivDescCreateRoutine']],
            ['Alter_routine', 'ALTER ROUTINE', $GLOBALS['strPrivDescAlterRoutine']],
            ['Execute', 'EXECUTE', $GLOBALS['strPrivDescExecute5']],
        ];
        // this one is for a db-specific priv: Create_view_priv
        if (isset($row['Create_view_priv'])) {
            $privTable[1][] = ['Create_view', 'CREATE VIEW', $GLOBALS['strPrivDescCreateView']];
        }
        // this one is for a table-specific priv: Create View_priv
        if (isset($row['Create View_priv'])) {
            $privTable[1][] = ['Create View', 'CREATE VIEW', $GLOBALS['strPrivDescCreateView']];
        }
        if (isset($row['Event_priv'])) {
            // MySQL 5.1.6
            $privTable[1][] = ['Event', 'EVENT', $GLOBALS['strPrivDescEvent']];
            $privTable[1][] = ['Trigger', 'TRIGGER', $GLOBALS['strPrivDescTrigger']];
        }

        // a d m i n i s t r a t i o n
        $privTable[2] = [
            ['Grant', 'GRANT', $GLOBALS['strPrivDescGrant']],
        ];
        if ($db == '*') {
            $privTable[2][] = ['Super', 'SUPER', $GLOBALS['strPrivDescSuper']];
            $privTable[2][] = ['Process', 'PROCESS', $GLOBALS['strPrivDescProcess']];
        ...
        if ($db == '*') {
            ...
            $privTable[2][] = ['Create_user', 'CREATE USER', $GLOBALS['strPrivDescCreateUser']];
        }
...
if (isset($_REQUEST['delete']) || (isset($_REQUEST['change_copy']) && $_REQUEST['mode'] < 4)) {
    if (isset($_REQUEST['change_copy'])) {
        $selected_usr = [$old_username . $user_host_separator . $old_hostname];
    } else {
